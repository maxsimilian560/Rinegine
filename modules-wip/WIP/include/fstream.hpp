...
namespace Rinegine::WIP {
  namespace File {
    ...
    inline constexpr FLAG& operator|=(FLAG& lhs, unsigned rhs) noexcept { return lhs = static_cast<FLAG>(static_cast<unsigned>(lhs) | rhs); }
    inline Rinegine::Stream open(const char* path, uint32_t flags);
    static Rinegine::StreamResult read_raw(void* handle, void* dst, size_t len);
    static Rinegine::StreamResult write_raw(void* handle, const void* src, size_t len);
    ...
    Stream open(const char* path, uint32_t flags) {
      ...
      if (fd < 0) {
        Stream s{};
        Kernel::Flags::set(s.flags, STREAM_ERROR);
        return s;
      }
      ...
      s.flags = 0;
      if (flags & File::FLAG::READ)  Kernel::Flags::set(s.flags, STREAM_READABLE);
      if (flags & File::FLAG::WRITE) Kernel::Flags::set(s.flags, STREAM_WRITABLE);
      s.buffer = nullptr;
      s.ops = &ops;
      return s;
    }
