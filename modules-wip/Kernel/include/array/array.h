...
template <class type> class Array : protected RawArray {
public:
  //[CONSTRUCT]
  Array() = default;
  ...
  Array(const Array &other) {
    ...
    if (size() > 0) [[likely]] {
      if constexpr (Rinegine::Kernel::Util::is_trivial_v<type>) {
        ...
        for (size_t i = 0; i < size(); ++i) {
          new (Rinegine::Kernel::Util::addressof(data()[i]))
              type(other.data()[i]);
        }
      }
    }
  ...
  explicit Array(size_t capacity) { resize(capacity); }
  //[operators]
  ...
  //[main ptr]
  type *data() noexcept { return reinterpret_cast<type *>(RawArray::data); }
  ...
  //[capacity]
  size_t size() const noexcept { return RawArray::size / sizeof(type); }
  ...
  type *end() noexcept {
    return reinterpret_cast<type *>(RawArray::data) + RawArray::size;
  }
  ...
  void reserve(size_t n_elements) {
    size_t nreal_size =
        low_level::align_to_cache_line(n_elements * sizeof(type));
    if (RawArray::real_size >= nreal_size) [[unlikely]]
      return;
    ...
    if constexpr (Rinegine::Kernel::Util::is_trivial_v<type>) {
      ...
      if (RawArray::data != nullptr) [[unlikely]] {
        for (size_t i = 0; i < current_size; ++i) {
          new (Rinegine::Kernel::Util::addressof(new_objects[i]))
              type(std::move(data()[i]));
          data()[i].~type();
        }
  ...
  void resize(size_t n_elements, const type &in) {
    ...
    if (nsize < prevsize) {
      ...
      if constexpr (Rinegine::Kernel::Util::is_trivially_copyable_v<type>) {
  ...
  void push_back() {
    ...
    new (static_cast<void *>(Rinegine::Kernel::Util::addressof(
        data()[current_size]))) type(std::move(type()));
  ...
  void push_back(type value) {
    ...
    new (static_cast<void *>(Rinegine::Kernel::Util::addressof(
        data()[current_size]))) type(std::move(value));
    RawArray::size += sizeof(type);
  }
  ...
  template <typename... Args> type &emplace_back(Args &&...args) {
    ...
    type *placed_object = ::new (static_cast<void *>(
        Rinegine::Kernel::Util::addressof(data()[current_size])))
