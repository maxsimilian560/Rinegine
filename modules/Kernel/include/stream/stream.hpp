  ...
  namespace Kernel {
    ...
    struct Stream {
      ...
      constexpr StreamResult read(void* dst, size_t len) {
        if (!Kernel::Flags::has(flags, STREAM_READABLE)) {
          Kernel::Flags::set(flags, STREAM_ERROR);
          return { 0, -1 };
        }
        if (Kernel::Flags::has(flags, STREAM_ERROR)) return { 0, -1 };
        ...
        if (!buffer) {
          ...
          if (r.bytes == 0 && r.err == 0) Kernel::Flags::set(flags, STREAM_EOF);
          if (!r.ok()) Kernel::Flags::set(flags, STREAM_ERROR);
          return r;
        }
        ...
        while (total < len) {
          ...
          if (r.bytes == 0) {
            Kernel::Flags::set(flags, STREAM_EOF);
            return { total, 0 };
          }
      ...
      constexpr StreamResult seek(int64_t offset, int origin) {
        ...
        Kernel::Flags::clear(flags, STREAM_EOF);
        return { 0, 0 };
      }
